...
class _DoctorPortalPageState extends State<DoctorPortalPage> {
  ...
  Future<void> _initializeModules() async {
    ...
    final role = userProvider.currentUser?.role ?? 'doctor';
    
    // Check if modules are already initialized by checking if allModules is empty
    if (moduleProvider.allModules.isEmpty || 
        (role == 'doctor' && !moduleProvider.allModules.any((m) => m.id.startsWith('doctor_')))) {
  ...
  Widget _buildGreetingSection(BuildContext context, UserProvider userProvider) {
    final username = userProvider.currentUser?.name ?? 'Doctor';
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  Text(
                    'Ready to manage your practice? Explore your modules below and stay on top of your appointments and patient care.',
                    style: TextStyle(
                      fontSize: 14,
                      fontWeight: FontWeight.w500,
                      color: AppTheme.textSecondary,
  ...
  void _navigateToModule(BuildContext context, String moduleId) {
    switch (moduleId) {
      case AppConstants.moduleDoctorAppointments:
        context.go('/doctor/appointments');
        break;
      case AppConstants.moduleDoctorAvailability:
        context.go('/doctor/availability');
        break;
      case AppConstants.moduleDoctorScanQR:
        context.go('/doctor/scan-qr');
        break;
      case AppConstants.moduleDoctorProfile:
